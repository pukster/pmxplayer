...
// Moved this up as it is needed in OMXPlayerSync class
#define S(x) (int)(DVD_PLAYSPEED_NORMAL*(x))
int playspeeds[] = {
    S(0),            // 0  Playspeed slow min
    S(1/16.0),       // 1
    S(1/8.0),        // 2
    S(1/4.0),        // 3
    S(1/2.0),        // 4
    S(0.975),        // 5
    S(1.0),          // 6
    S(1.125),        // 7 Playspeed slow max
    S(-32.0),        // 8 Playspeed rew max
    S(-16.0),        // 9
    S(-8.0),         // 10
    S(-4),           // 11
    S(-2),           // 12
    S(-1),           // 13 Playspeed rew min
    S(1),            // 14 Playspeed normal
    S(2.0),          // 15 Playspeed ff min
    S(4.0),          // 16
    S(8.0),          // 17
    S(16.0),         // 18
    S(32.0)          // 19 Playspeed ff max
};
int playspeed_current;
const int playspeed_slow_min  = 0;
const int playspeed_slow_max  = 7;
const int playspeed_rew_max   = 8;
const int playspeed_rew_min   = 13;
const int playspeed_normal    = 14;
const int playspeed_ff_min    = 15;
const int playspeed_ff_max    = 19;
double stamp;
...
int main(int argc, char *argv[])
{
    ...
    const int sync_server_opt = 0x500;
    const int sync_port_opt = 0x501;
    const int sync_client_opt = 0x502;
    ...
    struct option longopts[] = {
        ...
        { "sync-server",  no_argument,        NULL,                sync_server_opt },
        { "server-port",  required_argument,  NULL,                sync_port_opt },
        { "sync-client",  no_argument,        NULL,                sync_client_opt },
    ...
    while ((c = getopt_long(argc, argv, "wiIhvkn:l:o:cslb::pd3:Myzt:rg", longopts, NULL)) != -1)
    {
        switch (c) 
        {
            ...
            case 'k':
                ...
                return EXIT_SUCCESS;
                break;
            case sync_server_opt:
                m_is_sync_server = true;
                break;
            case sync_port_opt:
                tcp_port = atoi ( optarg );
                break;
            case sync_client_opt:
                m_is_sync_client = true;
                break;
            case server_address_opt:
                m_sync_server_address = optarg;
                break;
            case sync_num_clients_opt:
                sync_num_clients = atoi ( optarg );
                break;
...
change_file:
    ...
    if ( m_is_sync_server )
    {
        omxPlayerSync = new OMXPlayerSync ( SYNC_SERVER, sync_num_clients, tcp_port );
        omxPlayerSync -> display ();
        omxPlayerSync -> start ();
    }
    else if ( m_is_sync_client )
    {
        omxPlayerSync = new OMXPlayerSync ( SYNC_CLIENT, tcp_port, m_sync_server_address );
        omxPlayerSync -> display ();
        omxPlayerSync -> start ();
    }
